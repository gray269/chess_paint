...
La version 0.5 contient une interface de secours intégrée directement dans `index.html`. Même si le module principal ne charge pas, les boutons **Effacer**, **Charger l’exemple** et **Coller le PGN** doivent réagir. Un message visible indique alors que GitHub Pages sert probablement les sources brutes. Dans **Settings → Pages**, la source doit être **GitHub Actions**, jamais **Deploy from a branch**.
